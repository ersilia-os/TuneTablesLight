...
def transformer_onnx_predict(
    ...
    **kwargs,
    ...
    def predict_onnx(
        ...
        style_np = np.zeros(0)
        src1_np  = eval_xs.cpu().numpy()
        src2_np  = eval_ys.float().cpu().numpy()

        seq_len = src1_np.shape[0]
        src_mask_np = np.zeros(0)
        pos_np = np.array([eval_position], dtype=np.int64)
        ...
        try:
            ...
            for name, arr in [
                ...
                try:
                    a = np.array(arr)
                    print(f"  {name}: OK, shape={a.shape}, dtype={a.dtype}")
                except Exception as sub_e:
                    print(f"  {name}: FAILED → {sub_e}")
            raise

        out = torch.from_numpy(out_np).to(eval_xs.device)
        out = out / torch.exp(softmax_temperature)
    ...
    for batch_input, batch_label in zip(inputs, labels):
        output_batch = predict_onnx(
            batch_input, batch_label, style_, softmax_temperature_, True
        )
...
def transformer_predict(
    ...
    **kwargs,
    ...
    def predict(eval_xs, eval_ys, used_style, softmax_temperature, return_logits):
        ...
        print(f"Input shape: {eval_xs.shape} and output shape: {eval_ys.shape}")
        print(f"Style: {used_style} | eval pos: {eval_position}")
...
def create_graph_input(X, y):
    y = expand_y(X, y)
    style_src = torch.randn(*X.shape, dtype=torch.float32)
    print(f"Inout shapes: {style_src.shape} | {X.shape} | {y.shape}")
    print(f"Inout dtyoes: {style_src.dtype} | {X.dtype} | {y.dtype}")
    return [style_src, X, y]


def expand_y(X, y):
    d = X.shape[0]-y.shape[0]
    y = np.concatenate((y, np.zeros((d, y.shape[1]))), axis=0)
    y = y[:, :, np.newaxis]
    y = np.repeat(y, X.shape[-1], axis=2)
...
class TuneTablesClassifierLight(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba_2(self, X, cat_idx=[]):
        assert isinstance(X, np.ndarray), "x must be a numpy array"
    ...
    def concate_train_data(self, X_test, x_train, y_train, eval_pos, no_grad=True):
        if no_grad:
            ...
            assert torch.is_tensor(x_train) & torch.is_tensor(x_train), (
                "If no_grad is false, this function expects X as "
                "a tensor to calculate a gradient"
            )
            X_full = (
                torch.cat((x_train[:eval_pos, ...], X_test), dim=0)
                .float()
                .unsqueeze(1)
                .to(self.device)
            )
        ...
        y_full = np.concatenate([y_train[:eval_pos, ...], np.zeros(shape=X_test.shape[0])], axis=0)
        y_full = torch.tensor(y_full, device=self.device).float().unsqueeze(1)

        return X_full, y_full
    
    def export(self, X, y, eval_pos):
        input = X[0, :].unsqueeze(0)
        output = y[0, :].unsqueeze(0)
        print("Shape before")
        print(input.shape)
        print(output.shape)
        _x = torch.zeros(1, 1, 70)
        input = torch.concat((input, _x), dim=2)
        # inputs = create_graph_input(input, output)
        print("Shape after")
        print(input.shape)
        print(output.shape)
        # print(f"Inputs shape: {len(inputs)}")
        wrapper = ONNXWrapper(self.model, eval_pos).eval()
    ...
    def predict_proba(
        self, X, normalize_with_test=False, return_logits=False, return_early=False, path=None
    ):
        if self.is_fitted:
            ...
            # self.model = ort.InferenceSession(os.path.join(path, "model.onnx"), providers=["CPUExecutionProvider"])

        # X_full, y_full = self.concate_train_data(X, self._x_train, self._y_train, MAX_EVAL_POS)


        eval_pos, use_batch = self._calculate_eval_pos(self._x_train, X)
        self.model.single_eval_pos = eval_pos
        # self.export(X_full, y_full, eval_pos)
        self.no_grad = True
...
class ONNXWrapper(torch.nn.Module):
    def __init__(self, orig, single_eval_pos):
        super().__init__()
        self.orig = orig
        self.orig.single_eval_pos = single_eval_pos
    def forward(self,src):
        return self.orig(src)